...
function srcinfo() {
  local CARCH='CARCH_REPLACE' DISTRO='ubuntu:jammy' var ar aars bar ars rar _distros _vars _archs _sums distros parsed_distros \
  ...
  mapfile -t parsed_distros < <(awk 'NF && !/:$/ {sub(/\/.*/, "", $1); print $1}' distrolist)
  distros="${parsed_distros[*]}"
